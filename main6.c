...
void save_info(char itname[ITEM][NAME], int* incoming, int* sale, int* price, int* judge);
void load_info(char itname[ITEM][NAME], int* incoming, int* sale, int* price, int* judge);
...
int main()
{
	...
	while (menu != 7)
	{
		printf("원하는 메뉴를 선택하세요.(1.입고, 2.판매, 3.개별상품정보, 4.전체상품정보, 5.상품정보저장, 6.상품정보가져오기, 7.종료)\n");
		scanf("%d", &menu);
		...
		case 5:
			save_info(itname, incoming, sale, price, judge);
			break;

		case 6:
			load_info(itname, incoming, sale, price, judge);
			break;
...
void save_info(char itname[ITEM][NAME], int* incoming, int* sale, int* price, int* judge)
{
	FILE* fp = fopen("inventory_data.bin", "wb");
	...
	fwrite(judge, sizeof(int), ITEM, fp);
	fwrite(incoming, sizeof(int), ITEM, fp);
	fwrite(sale, sizeof(int), ITEM, fp);
	fwrite(price, sizeof(int), ITEM, fp);
	fwrite(itname, sizeof(char), ITEM * NAME, fp);

	fclose(fp);
	printf("상품 정보가 'inventory_data.bin' 파일에 저장되었습니다.\n");
}

void load_info(char itname[ITEM][NAME], int* incoming, int* sale, int* price, int* judge)
{
	FILE* fp = fopen("inventory_data.bin", "rb");

	if (fp == NULL)
	{
		printf("저장된 파일을 찾을 수 없습니다.\n");
		return;
	}

	fread(judge, sizeof(int), ITEM, fp);
	fread(incoming, sizeof(int), ITEM, fp);
	fread(sale, sizeof(int), ITEM, fp);
	fread(price, sizeof(int), ITEM, fp);
	fread(itname, sizeof(char), ITEM * NAME, fp);
